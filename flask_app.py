...
holidays = USFederalHolidayCalendar().holidays(
    start=today, end=yearEnd, return_name=True
)
...
def mainPage():
    unanet_username = request.form.get("unanet_username") or request.cookies.get(
        "unanet_username"
    )
    unanet_password = request.form.get("unanet_password") or None
    hours_worked = request.form.get("hours_worked") or request.cookies.get(
        "hours_worked"
    )
    annual_minimum = (
        request.form.get("annual_minimum")
        or request.cookies.get("annual_minimum")
        or 1790
    )
    hours_per_day = (
        request.form.get("hours_per_day") or request.cookies.get("hours_per_day") or 8
    )
    include_today = request.form.get("include_today")
    blankPage = render_template(
        "main.html",
        days_off="",
        workable_days="",
        pyh_score="",
    ...
    if request.method == "POST" and hours_worked and annual_minimum and hours_per_day:
        ...
        leave_score = round(
            (days_off_remaining_percent - workable_days_percent) * 100, 2
        )
        pyh_score = round((hours_worked_percent - worked_days_percent) * 100, 2)
        resp = make_response(
            render_template(
                "main.html",
                days_off=f"{days_off_remaining}",
                pyh_score=f"{leave_score} / {pyh_score}",
                today=f"{todays_date} ({progress}%)",
                workable_days=f"{workable_days}",
...
def get_report(login, passwd):
    ...
    for td in tds[1:]:
        if len(td) == 3:
            results["data"].append(
                {
                    headers[0].string: td[0].string,
                    headers[1].string: td[1].string,
                    headers[2].string: td[2].string,
                }
            )
        elif len(td) == 2:
